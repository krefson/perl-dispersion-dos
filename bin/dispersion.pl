...
sub make_abscissa {
  ...
  for $q (@$qpts) {
    ...
    if( ! ((fabs($delta[0])+fabs($delta[1])+fabs($delta[2]) > $kv_tol &&
	   ...
	   $delta_rem[0]+$delta_rem[1]+$delta_rem[2] > $kv_jump_tol ) ){
	...
	if( $nq > 0) {
	    $modq=sqrt($deltaq[0]*$deltaq[0]+$deltaq[1]*$deltaq[1]+$deltaq[2]*$deltaq[2]);
	    #
	    # Adaptation of "break" keyword jump detection tolerance. 8 * the increment for this path should do.
	    if( ($deltaq[0]+$deltaq[1]+$deltaq[2]) > $kv_tol &&
		$kv_jump_tol > 8*($deltaq[0]+$deltaq[1]+$deltaq[2]) ) {
		$kv_jump_tol = 8*( $deltaq[0]+$deltaq[1]+$deltaq[2] );
		printf STDERR "INFO: Resetting k-path jump detection tolerance to %f\n", $kv_jump_tol;
	    }
	} 
    }
    $qtot += $modq;
